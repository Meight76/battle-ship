...
export default class GameBoard {
    ...
    #initializeBoard() {
        return this.#board = Array(10).fill().map(() => new Array(10).fill().map(() => new Square()));
    }
    ...
    placeShip(shipLengh, coordArr, axeMode) {
        ...
        if (!(Array.isArray(coordArr)) || coordArr.length !== 2) throw new Error("invalid coord for deploy");
        // declare coord variable
        const [y, x] = coordArr;
        // check if coords are in range
        if (x > 9 || x < 0 || y > 9 || y < 0) throw new Error("coords out of range");
        // check if mode is string
        if (typeof axeMode !== "string") throw new Error("invalid axe provided");
        ...
        if (axeMode.charAt(0).toLowerCase() === "v") {
            if (x + shipObj.length > 10) throw new Error("not enough space to fit vertically ship");
        ...
        // mark all the horizontal with reference to shipObj
            ...
            if (y + shipObj.length > 10) throw new Error("not enough space to fit horizontally");
    ...
    receiveAttack(coordArr) {
        if (!(Array.is(coordArr)) || coordArr.length !== 2) throw new Error("invalid coord provided to receive attack");
        // this.#board[]
    }

    /* debug purpose */
    getSquare(coordArr) {
        if (!(Array.isArray(coordArr))) throw new Error("invalid coord array to retrieve square");
...
gameBoard.placeShip(4, [1, 4], "v");
gameBoard.visualizeBoard();
console.log(gameBoard.getSquare([1, 6]).shipPointer); */
